...
from ocr.src.tools import Constants
...
class ImageAnalyzer:
    ...
    def __init__(
        ...
        self._ocr_engine = OCRMLP(weights_path=Constants.MLP_WEIGHTS_PATH_HEX)
        self._image = None
    ...
    def run_without_stream(self, screenshot_on_pause=False):
        self._image_capture.start_recording()
        while True:
            image = self._image_capture.get_image()
            key = cv2.waitKey(5)
            if self._is_oled_on_screen(image):
                if screenshot_on_pause and self._is_oled_on_screen(image):
                    self.save_image(image)
                    cropped = self.crop_image(image)
                    text = self.extract_text(cropped)
    ...
    def extract_text(self, image):
        ...
        if type(image) == str:
            image = cv2.imread(image)
        text = self._ocr_engine.get_text(image, verbose=False)
        return text
